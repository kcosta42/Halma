...
/*   Updated: 2016/12/29 15:51:30 by kcosta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
												...
												t_vector pion_selected, int redo)
{
	...
	while (pos.y < 16)
	{
		...
		while (pos.x < 16)
		{
			...
			if (current != '.')
			{
				if (pos.x == pion_selected.x && pos.y == pion_selected.y)
					printf("%s %c%s", (redo & 1) ? BLUE : EOC, current, EOC);
				else
					printf(" %c", current);
			}
			else
	...
	sleep(1);
	blink_pion(nb_player, player, pion_selected, redo - 1);
}
